...
<div id="bnt_poll_answers"></div>\
  ...
  topicFieldUpdated: function() {
    var newt = val(cur.addField), newh = getSize(cur.addBlock)[1];
  ...
  replyPost: function(post, mid) {
    if (browser.mobile) return elfocus(cur.addField);
    cur.addForce = true;
    Board.topicOnScroll(false, false, true);
    ...
    if (document.selection) {
      ...
      if (range.parentElement() == cur.addField) {
        ...
        val(cur.addField, cur.addText + insert);
        pos = cur.addText.length + insert.length;
      }
    } else {
      pos = cur.addField.selectionEnd + insert.length;
      cur.addField.blur();
      val(cur.addField, cur.addText.substr(0, cur.addField.selectionStart) + insert + cur.addText.substr(cur.addField.selectionStart));
    }
    if (pos >= 0) {
      elfocus(cur.addField, pos, pos);
    }
...
<div class="bte_labeled fl_l">\
  ...
  initTopic: function(opts, preload) {
    ...
    if (cur.addField) {
      ...
      autosizeSetup(cur.addField, {minHeight: 42, maxHeight: 160});
  ...
  topicOnScroll: function(e, st, pp) {
    ...
    var needDock = (cur.addText || cur.addForce || cur.boardAddMedia.chosenMedia) && (st + lastWindowHeight < cur.addBlockTop + cur.addBlockHeight);
  ...
  sendPost: function() {
    ...
    var value = trim(val(cur.addField)),
...
<textarea id="bpe_text" onkeyup="checkTextLength(cur.postLimit, this, \'bpe_warn\')" onkeypress="onCtrlEnter(event, Board.savePost)">' + text + '</textarea>\
<div id="bpe_warn"></div>\
