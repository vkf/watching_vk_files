...
switchTab: function(name, evt) {
  ...
  var oldTab = false;
  each(geByClass('active_link', ge('bugs_tabs')), function(i, v) {
  ...
  if (name == 'show') {
    ...
    if (evt === false) {
      ...
      return nav.go(ge(name + '_tab').firstChild, evt);
    }
  }
},

switchSection: function(name, evt) {
  if (checkEvent(evt)) return true;
...
getPage: function(offset) {
  ...
  ajax.post('bugs', query, {
    cache: 1,
    onDone: function(content, script, summary, tags) {
...
subscribeBug: function(btn, subscribe) {
  ajax.post('bugs', {act: 'subscribe', id: cur.bug_id, subscribe: subscribe, hash: cur.hashes.subscribe_hash}, {
    onDone: function(html) {
      if (html) ge('bugs_view_subscribe').innerHTML = html;
    },
...
getSearchParams: function(obj) {
  ...
  if ((params.q || params.tags) && nav.objLoc != 'all' && nav.objLoc.act != 'my') {
    nav.objLoc.act = 'all';
  }
...
updateAllSearch: function(obj, delay) {
  delay = delay || 10;
...
searchAll: function() {
  ...
  ajax.post('bugs', query, {
    cache: 1,
    onDone: function(cont, script, summary, tags) {
      ...
      toggle('tags_wrap', !!tags);
      delete nav.objLoc.offset;
      each(['q', 'status', 'tags'], function(i, v) {
        if (query[v]) {
...
saveBug: function(bug_id) {
  if (!this.checkNewFields()) return false;
