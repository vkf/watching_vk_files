...
//    debugLog('feed updated', data);
      ...
      case 'new_post':
        ...
        if (place) { // Client-side grouping posts
          ...
          if (grouped = geByClass1('feed_reposts_wrap' + place, cont, 'div')) {
            ...
            var frow = se('<div class="feed_row">' + postHTML + '</div>');
  ...
  applyOptions: function(options, from) {
    ...
    if (cur.section == 'notifications' && cur.notify) {
  ...
  showMore: function() {
    ...
    extend(params, {offset: cur.offset, from: cur.from, part: 1, more: 1});
  ...
  notifyCheckHideReply: function (item, target) {
    ...
    do {
      if (!target || (foundGood = (target.id == (item + '_reply_wrap') || target.id == ('feedback_row_' + item)))) {
        break;
      }
    ...
    cur.editing = false;
    if (!opts.disabled) {
      var ta = ge(item + '_field'), msg = trim(val(ta));
      if (msg && !opts.greet || opts.greet.indexOf(msg)) {
        return;
      }
    }
  ...
  notifyDelete: function (item, types, hash, el) {
    el.tt && el.tt.el && el.tt.hide();
    ajax.post('al_feed.php', {act: 'a_feedback_delete', item: item, hash: hash, types: types}, {
  ...
  editHidden: function () {
    showTabbedBox('al_settings.php', {act: 'a_edit_owners_list', list: 'feed', height: lastWindowHeight}, {stat: ['privacy.js', 'privacy.css', 'ui_controls.js', 'ui_controls.css', 'indexer.js']});
  ...
  editList: function (list_id) {
    Feed.toggleTabsMenu(0);
    showTabbedBox('al_settings.php', {act: 'a_edit_owners_list', list: 'feed', list_id: list_id, height: lastWindowHeight}, {stat: ['privacy.js', 'privacy.css', 'ui_controls.js', 'ui_controls.css', 'indexer.js']});
  ...
  deleteList: function (list_id, force) {
    ...
    if (!force) {
      var box = showFastBox(getLang('news_delete_list_confirm_title'), getLang('news_delete_list_confirm'), getLang('box_yes'), function () {
  ...
  searchChooseGeoPoint: function () {
    ...
    cur.chooseGeoPoint = function (lat, lon, zoom) {
      ...
      setStyle(map, {
        background: 'url(//maps.googleapis.com/maps/api/staticmap?center='+lat+','+lon+'&zoom='+zoom+'&size='+(window.devicePixelRatio >= 2 ? '260x160' : '130x80')+'&sensor=false)'
      });
  ...
  searchUrlOnChange: function (fromTooltip) {
    var urlMatches = (val('feed_searchext_link') || '').match(urlInactiveExp),
        newTag = '';

    if (urlMatches) {
      var url = urlMatches[4] + (urlMatches[5] || ''),
          domain = urlMatches[4],
  ...
  searchContentOnChange: function () {
    var urlMatches = (val('feed_searchext_content') || '').match(urlInactiveExp),
  ...
  sinceOwnerBigphOver: function(obj, oid) {
    ...
    animate(o, {marginTop: 65}, {duration: 200, transition: Fx.Transitions.easeOutCirc});
    cur.phShown[oid] = o;
