...
checkTwitter: function() {
  showBox('al_profileEdit.php', {act: 'twitter_settings_box', import_friends: 1});
},
...
checkOAuth: function(googleLang, state, type) {
  ...
  if (type == 1) {
    var oauthUrl = 'https://accounts.google.com/o/oauth2/auth?scope=https://www.google.com/m8/feeds/&response_type=code&redirect_uri='+encodeURIComponent(redirectUri)+'&approval_prompt=force&state='+state+'&client_id=198946236159.apps.googleusercontent.com&hl='+googleLang;
  } else if (type == 3) {
...
checkImportResult: function() {
  var box = curBox();
  ajax.post('al_friends.php', {act: 'check_contacts_import'}, {
    onDone: FriendsSearch.onImportResult,
    showProgress: box ? box.showProgress : false,
    hideProgress: box ? box.hideProgress : false,
    stat: ['intro.css']
  });
},
...
checkImportingLoop: function() {
  ...
  cur.importingInt = setInterval(function() {
    ajax.post('al_friends.php', {act: 'check_contacts_import', provider: 'twitter'}, {
      onDone: function(data, lng, importedList) {
        if (!data) {
          return false;
        }
        clearInterval(cur.importingInt);
        box.hide();
        FriendsSearch.onImportResult(data, lng, importedList);
      },
      onFail: function() {
        clearInterval(cur.importingInt);
      }
