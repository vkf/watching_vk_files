...
' + (tip ? '<div class="gs_tip">' + tip + '</div>' : '') + '\
  ...
  initGiftsBox: function(box, opts) {
    ...
    extend(cur, {
      ...
      gftbxMid: opts.mid,
      gftbxId: box.tbId,
      gftbxAddLnk: ge('tb_'+box.tbId+'_prg').parentNode.insertBefore(se('<div class="gifts_add_lnk fl_r"><a href="/gifts?act=send" onclick="return (window.Profile ? Profile.showGiftBox(cur.oid, event) : nav.go(this, event));">' + getLang('gifts_send_gift') + '</a></div>'), ge('tb_'+box.tbId+'_prg')),
