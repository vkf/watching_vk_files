var GroupsEdit = {
  ...
  uApproveAll: function() {
    var box = showFastBox(
      ...
      function() {
        ajax.post('groupsedit.php', {act: 'approve_all', id: cur.opts.id, hash: cur.opts.hash}, {
  ...
  uShowMore: function(force) {
    ...
    if (!d || d == 'loading' || d == 'notavail' && force) return;
  ...
  uGenRow: function(tab, row, name) {
    ...
    if (infoArr[1]) { // online
      info += '<div class="gedit_user_online">' + (getLang('global_online', sex) + (infoArr[1] != 1 ? '<b class="mob_onl gedit_mob_onl" onmouseover="mobileOnlineTip(this, {mid: ' + oid + '})" onclick="mobilePromo()"></b>' : '')) + '</div>';
    }

    switch (tab) {
      case 'requests':
        if (level != -1) { // declined or approved request
          btns = [
...
'</div>'].join('');
  ...
  uEditAdmin: function(user) {
    showBox('groupsedit.php', {act: 'edit_admin', id: cur.opts.id, addr: user});
  },
  uRemoveAdmin: function(user) {
    return showBox('groupsedit.php', {act: 'edit_admin', id: cur.opts.id, addr: user, remove: 1});
  },
  ...
  uMainAdmin: function() {
    showBox('groupsedit.php', {act: 'main_admin', id: cur.opts.id});
  },
...
<center id="group_status_export_data">' + getLang('groups_external_site_request') + '</center>\
  ...
  removeTwitter: function() {
    var box = showFastBox(getLang('groups_status_export'), '<center>' + getLang('groups_status_confirm') + '</center>', getLang('global_continue'), function() {
  ...
  nbAddr: function() {
    var tbl = ge('group_edit_addr_table');
  ...
  editLink: function(id) {
    showBox('al_groups.php', {act: 'edit_link', lid: id, gid: cur.gid, hash: cur.hash});
  },
  ...
  addLink: function() {
    ...
    ajax.post('al_groups.php', {act: 'add_link_box', lnk: lnk}, {onDone: function(code, innerlnk, html, js) {
      if (code > 0) {
        ...
        cur.lnk = innerlnk;
        cur.lnkOwnerId = cur.lnkPhotoId = false;
        cur.addBox = showFastBox(cur.boxTitle, html, getLang('global_add'), GroupsEdit.doAddLink, getLang('global_cancel'));
