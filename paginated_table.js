...
window.PaginatedTable.prototype._constructFooter = function() {
  ...
  for (i = 0; i < content.footer.length; i++) {
    var text = '', sum, cur, num;
    switch (content.footer[i]) {
      case 'none':
        break;
      case 'sum':
        sum = 0;
      ...
      case 'average':
        ...
        for (j = 0; j < content.data.length; j++) {
          cur = Number(content.data[j][i]);
          if (!isNaN(cur) && (cur != 0 || !this.options.behaviour.SKIP_ZEROS_ON_AVERAGE)) {
            sum += cur;
            num++;
          }
        }
      ...
      case 'label':
        ...
        break;
      default:
        text = content.footer[i];
        break;
    }
    if (content.footer[i] != 'none' && content.footer[i] != 'label' && _t.options.columnFormatting && _t.options.columnFormatting[i]) {
