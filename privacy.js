var Privacy = {
  ...
  hideFriends: function(key, obj) {
    ...
    showBox('al_friends.php', {act: 'select_friends_box', from: 'friends_privacy', Checked: checked.join(',')}, {stat: ['privacy.js', 'privacy.css', 'indexer.js']});
  ...
  customSaved: function(key, np, plus, minus) {
    ...
    if (np[1] == 1 && !np[3].length || np[0] == Privacy.listsType) {
      ...
      if (key == 'hidden_friends') { // hack for hidden friends
      ...
      if (minus.length) {
        ...
        for (var i = 0; i < count && i < 5; ++i) {
          ...
          if (id > 0) {
            ...
            var cat_id = -id, color = (cat_id - 1) % 8 + 1;
            mstr.push('<span class="group' + color + '">' + lists[cat_id] + '</span>');
          }
        }
  ...
  choose: function(ev, val, list) {
    ...
    if (val == Privacy.customType) {
      ...
      return showBox('al_friends.php', {act: 'custom_privacy_box', type: type, plus: plus.join(','), minus: minus.join(','), opt: cur.privacy.custom_box_type ? cur.privacy.custom_box_type : (key == 'apps_invite' || key == 'videocalls' ? 'fronly' : '')}, {stat: ['ui_controls.js', 'ui_controls.css']});
