window['public'] = window.Public = {
  ...
  showLinks: function() {
    var b = showBox('/al_public.php', {act: 'a_get_links', pid: cur.options.public_id}, {params:{width:467}});
  ...
  showEvents: function(edit) {
    var b = showBox('/al_public.php', {act: 'a_get_events', pid: cur.options.public_id, edit: edit}, {params:{width:467}});
    b.setOptions({onHideAttempt: function() {
      if (cur.reloadAfterClose) {
        nav.reload({noscroll: true});
        cur.reloadAfterClose = false;
  ...
  showApps: function(edit) {
    var b = showBox('/al_public.php', {act: 'a_get_apps', pid: cur.options.public_id, edit: edit}, {params:{width:467}});
    b.setOptions({onHideAttempt: function() {
      if (cur.reloadAfterClose) {
        nav.reload({noscroll: true});
        cur.reloadAfterClose = false;
  ...
  subscribe: function() {
    ...
    ajax.post('al_public.php', {act:'a_enter', pid:cur.options.public_id, hash:cur.options.enterHash}, {
      ...
      onFail: function(text) {
        if (!text) return;
        showFastBox({title: getLang('global_error'), onHide: nav.reload}, text);
        return true;
      }
    });
    return false;
  ...
  editEvent: function(pid, eid, hash) {
    var b = showBox('al_page.php', {act:'a_edit_event_box', pid:pid});
  },
  ...
  showPlaces: function(edit) {
    var b = showBox('al_public.php', {act: 'a_get_places', pid: cur.options.public_id, edit: edit}, {
      params: {width: 467, progress: 'qwerty'},
