...
var searcher = {
  ...
  updResults: function(data) {
    ...
    if (searcher.sameParams(params)) return false;
  ...
  sendSearchReq: function (params, changeSection) {
    ...
    cur.searchReq = ajax.post('al_search.php', params, {
      onDone: function (options, rows, sections, filters, ads) {
  ...
  subscribe: function(el, oid, hash, sub) {
    var address, params;
    if (sub) {
      address = 'al_feed.php';
      params = {act: 'subscr', oid: oid, hash: hash};
    } else {
      address = 'al_fans.php';
