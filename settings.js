var Settings = {
  ...
  saveSmsNotify: function(btn) {
    ...
    ajax.post('al_settings.php', params, {
      onDone: function(result, html, href) {
        unlockButton(btn);
        if (html && href) {
          showFastBox(result, html, getLang('settings_subscribe_to_service_btn'), function() {
            window.open(href);
            curBox().hide();
  ...
  smsUnsubscribe: function(lnk, oid, hash, ev) {
    ...
    showFastBox(getLang('settings_confirm_unsubscribe_title'), msg, getLang('box_yes'), function() {
...
<form action="' + vk.loginscheme + '://login.vk.com/" method="POST" target="reset_sessions_frame">\
  ...
  <input name="reset_hash" value="' + cur.options.reset_hash + '" type="hidden" />\
  ...
  doCheckAddress: function () {
    var progressEl = ge('settings_address_progress'),
  ...
  init: function () {
    ...
    var laWrap = ge('settings_last_activity_wrap');
    if (laWrap) {
      laWrap.onmouseover = function () {
        showTooltip(this, {
          shift: [2, 3, 3],
          text: cur.options.last_activity_ip,
          hasover: 1,
          black: 1
        });
      }
