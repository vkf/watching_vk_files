...
addTask: function() {
  return !showBox(this.address, {act: 'edit'}, { params: {width: '520px'} });
},

editTask: function(bid) {
  return !showBox(this.address, {act: 'edit', bid: bid}, { params: {width: '520px'} });
},
...
deleteTask: function(bid, hash) {
  var box = showFastBox({title: cur.lang['delete_title'], width: 430, onHide: function() { if (cur.deleted) nav.go(Tasks.address);}}, cur.lang['delete_confirm'], cur.lang['delete'], function() {
...
editPriority: function(bid, hash) {
  var box = showFastBox({title: getLang('tasks_increase_priority'), width: 430}, getLang('tasks_sure_increase_priority'), getLang('tasks_increase'), function(btn) {
...
showPhoto: function(url, photo_id) {
  ...
  pic.src = url;
  var onload = function() {
    showFastBox({bodyStyle: "line-height: 0px;", width: Math.min(pic.width, 1024) + 30}, '<img class="tasks_preview_img" src="' + url + '" />');
    hide('tasks_loading');
...
changeTaskStatus: function(status, hash) {
  ...
  if (status == 1 && cur.binded_tickets && cur.binded_tickets.length) {
    var box = showFastBox({title: cur.lang['close_task'], width: 430, bodyStyle: "line-height: 160%;"}, cur.lang['sure_close'], getLang('global_close'), function() {
        box.changed = true;
    ...
    var box = showFastBox({title: getLang('tasks_please_wait'), width: 430, bodyStyle: "line-height: 160%;"}, '<div id="tasks_status_progress_wrap" class="tasks_upload_progress_wrap" style="margin: 20px auto"><div id="task_status_progress" class="tasks_upload_progress" style="width: 0%;"></div></div>', getLang('global_cancel'), function() {
        cur.forceStop = true;
        box.hide();
      }
    );
...
addAutoScreen: function() {
  var doShow = function() {
    var box = showBox('/helpdesk', {act: 'choose_photo_box', scrollbar_width: window.sbWidth()}, {params: {bodyStyle: 'padding: 0px'}, cache: 1, onFail: function() {show('tasks_sure_close_error'); hide('tis_add_lnk'); return true;}});
  }
  if (window.Tickets) {
    doShow();
  } else {
  ...
  cur.onFlistSave = function(ids, list, hash) {
    ...
    ajax.post(Tasks.address, {act: 'notify_users', ids: ids.join(','), bid: cur.bug_id, hash: hash}, {
      onDone:function(header, content) {
        box.removeButtons();
        box.addButton(global_cancel, box.hide, 'yes');
...
notifyUser: function(id, hash) {
  showBox(Tasks.address, {act: 'notify_users', ids: id, bid: cur.bug_id, hash: hash}, { params: {width: '430px'}});
},
...
bindTicket: function() {
  return !showBox(Tasks.address, {act: 'bind_ticket'}, {width: '430px'});
},
...
unbindTicket: function(ticket_id, hash) {
  ...
  cur.unbindBox = showFastBox({title: cur.lang['delete_ticket_bind'], width: 430, onHide: function() {
