var FullWall = {
  ...
  initOnePost: function(opts, preload) {
    ...
    if (cur.addField) {
      ...
      autosizeSetup(cur.addField, {minHeight: opts.note ? 62 : 42, maxHeight: 300});
  ...
  sendReply: function() {
    var rf = ge('fwr_text'),
        ...
        params = composer ? Composer.getSendParams(composer, FullWall.sendReply) : {message: trim(val(rf))},
    ...
    ajax.post('al_wall.php', params, {
      ...
      showProgress: lockButton.pbind(ge('fwr_send')),
      hideProgress: unlockButton.pbind(ge('fwr_send'))
    });
  },
  ...
  replyAction: function(act, post, hash) {
    if (cur.pid) {
    ...
    ajax.post('al_wall.php', {act: act, post: post, hash: hash, from: 'full', root: root ? 1 : 0}, {onDone: function(msg, deleted) {
      var p = root ? ge('fw_post') : ge('post' + post);
      if (!p) return;
      if (root) {
        cur.pgPaused = true;
