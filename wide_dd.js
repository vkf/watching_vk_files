...
<div class="' + cl + '" onmousedown="WideDropdown.select(\'' + dd.id + '\', event)" onmouseover="WideDropdown.over(\'' + dd.id + '\', \'' + clean(id) + '\')" id="wddi' + id + '_' + dd.id + '" onclick="">\
  ...
  _showList: function(dd) {
    ...
    dd.listWrap.style.marginTop = getSize(dd.listWrap.parentNode)[1] + 'px';
    if (!isVisible(dd.listWrap)) {
  ...
  _updateImgs: function(dd, noAnim) {
    ...
    for (var k in dd.selected) {
      var item = dd.selected[k], img = item[3], href = item[4], onl = item[5], ids = item[6];
      if (isArray(img)) {
...
<div class="wdd_add2">\
  <table cellspacing="0" cellpadding="0"><tr>\
    ...
    dd.listWrap = el.insertBefore(ce('div', {className: 'wdd_lwrap', innerHTML: '<div class="wdd_list"></div><div class="wdd_bottom1"></div><div class="wdd_bottom2"></div>'}, {display: 'none', width: opts.width || getSize(el)[0]}), el.firstChild);
    dd.list = dd.listWrap.firstChild;
